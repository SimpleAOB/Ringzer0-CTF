...
//^^
//SHA1 JavaScript Library found online
//

var PAGE = "";
var str = $("div.message").text(); //Get <div class="message"></div> text
str = str.replace(/(\r\n|\n|\r|\s)/gm,""); //Remove all line breaks, spaces, etc.
str = str.replace("-----BEGINHASH-----",""); //Remove Begin hash text
str = str.replace("-----ENDHASH-----",""); //Remove End hash text
var clrText = ""; //Initialize variable

for (var i = 0;i<10000;i++){ //The given hashes have a known value, and that value is 0-10000
	var shaObj = new jsSHA("SHA-1", "TEXT"); //Initilize the SHA1 hash object
	shaObj.update(i.toString()); //Update the object with the current iteration of i
	var hash = shaObj.getHash("HEX"); //Get the hex output of the hash
	if (hash == str) {clrText = i;break;} //Break if found
}



$.ajax({url: "https://ringzer0team.com/challenges/56/"+clrText, success: function(result){
    PAGE = result; //Read PAGE variable to see if you got the flag
}});
